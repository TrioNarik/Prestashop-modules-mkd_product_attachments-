...
<div class="product-attachments">
    ...
    <ul class="nav nav-tabs" role="tablist">
        {foreach from=$allUserGroups item=groupName key=groupKey}
            ...
            {*** Sprawdź, czy grupa ma przypisane załączniki ***}
            {assign var="hasAttachments" value=false}
            {foreach from=$attachments item=groupedAttachment}
                {if in_array($groupName, $groupedAttachment.group_info.user_groups)}
                    {assign var="hasAttachments" value=true}
                    {break}
                {/if}
            {/foreach}

            {*** Wyświetl tylko grupy z załącznikami ***}
            {if $hasAttachments && !in_array($groupKey, $turnOffForGroups)}
                {if $firstActiveTabGroupClass === null}
